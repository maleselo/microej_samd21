...
/*
 * Define an image with a LUT. The image format is equal to LLDISPLAY_UTILS#IMAGE_L4
 * or LLDISPLAY_UTILS#IMAGE_L8.
 */
typedef struct
{
	uint16_t width;		/* Image width in pixels */
	uint16_t height;	/* Image in pixels */
	uint8_t format;		/* Image format */
	uint8_t reserved;	/* reserved */
	uint8_t lut_format;	/* LUT format */
	uint8_t lut_size;	/* LUT size in bytes */
} LLDISPLAY_SImageLUT;
 ...
 * is specified when the source image is an "Alpha" image (format LLDISPLAY_UTILS#IMAGE_L4
 * or LLDISPLAY_UTILS#IMAGE_A8); otherwise this field is useless.
